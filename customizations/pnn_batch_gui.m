...
%       addpath('customizations')
    ...
    methods (Access = private)
        ...
        % Read current control values and persist all preferences.
            ...
            obj.P.useRescore      = obj.hUseRescore.Value;
            obj.P.detModelIdx     = find(strcmp(obj.hDetList.Items, obj.hDetList.Value), 1);
            obj.P.rescoreModelIdx = find(strcmp(obj.hRescoreList.Items, obj.hRescoreList.Value), 1);
        ...
        function buildGUI(obj)
            ...
            rootGrid.RowHeight  = {140, '1x', 130, 168, 142, 52};
            ...
            %% pModels
            pModels = uipanel(midGrid, 'Title', 'Models');
            pModels.Layout.Column = 2;
            gMod = uigridlayout(pModels, [5 1]);
            gMod.Padding    = [6 6 6 6];
            gMod.RowSpacing = 4;
            gMod.RowHeight  = {18, '1x', 22, 18, '1x'};

            lbl = uilabel(gMod, 'Text', 'Detection Model  (required):', ...
                'HorizontalAlignment', 'left', 'FontWeight', 'bold');
            lbl.Layout.Row = 1; lbl.Layout.Column = 1;

            detStrs = obj.detModels;
            if isempty(detStrs), detStrs = {'(none found)'}; end
            obj.hDetList = uilistbox(gMod, 'Items', detStrs, ...
                'Value', detStrs{min(obj.P.detModelIdx, numel(detStrs))}, ...
                'Tag', 'detList', 'FontSize', 11, ...
                'ValueChangedFcn', @obj.onDetModelChange);
            obj.hDetList.Layout.Row = 2; obj.hDetList.Layout.Column = 1;

            obj.hUseRescore = uicheckbox(gMod, ...
                'Text', 'Use Rescoring Model (Stage 2 — ConvNet re-ranking)', ...
                'Value', logical(obj.P.useRescore), 'Tag', 'useRescore', ...
                'Tooltip', 'Applies a second ConvNet model to re-score each detection', ...
                'ValueChangedFcn', @obj.onUseRescoreChange);
            obj.hUseRescore.Layout.Row = 3; obj.hUseRescore.Layout.Column = 1;

            rescoreEn = logical(obj.P.useRescore);
            obj.hRescoreLabel = uilabel(gMod, 'Text', 'Rescoring Model  (optional):', ...
                'HorizontalAlignment', 'left', 'FontWeight', 'bold', 'Enable', rescoreEn);
            obj.hRescoreLabel.Layout.Row = 4; obj.hRescoreLabel.Layout.Column = 1;

            rescoreStrs = obj.rescoreModels;
            if isempty(rescoreStrs), rescoreStrs = {'(none found)'}; end
            obj.hRescoreList = uilistbox(gMod, 'Items', rescoreStrs, ...
                'Value', rescoreStrs{min(obj.P.rescoreModelIdx, numel(rescoreStrs))}, ...
                'Tag', 'rescoreList', 'FontSize', 11, 'Enable', rescoreEn, ...
                'ValueChangedFcn', @obj.onRescoreModelChange);
            obj.hRescoreList.Layout.Row = 5; obj.hRescoreList.Layout.Column = 1;

            %% ---- Row 3 — predict.py Options ----------------------------
            ...
            %% ---- Row 4 — Multi-page & Preprocessing --------------------
        ...
        % Run a quick synchronous check that Python + required modules are importable.
            ...
            if ~isempty(condaEnv)
                if isempty(condaExe)
                    obj.hProgressLabel.Text = 'Conda env set but conda executable path is empty — browse for conda.exe';
                    return;
                end
                cmd = sprintf('"%s" run --no-capture-output -n %s %s -c "%s"', ...
                    condaExe, condaEnv, pyExe, testCode);
            else
                cmd = sprintf('"%s" -c "%s"', pyExe, testCode);
            end
            ...
            if status == 0
                fprintf('[TEST] PASS: %s\n', result);
                obj.hProgressLabel.Text = ['Environment OK: ' result];
            else
                fprintf('[TEST] FAIL (exit %d):\n%s\n', status, result);
                obj.hProgressLabel.Text = sprintf('Environment test FAILED (exit %d) — see Command Window', status);
            end
        end
